(function (factory) {
    ...
    */
    ...
    const getEventTarget = (e) => e.detail || e.target;
    const getValueFromEventTarget = (target) => {
    ...
    const IS_LITE_FORM = 'data-isliteform';
    const EVENTS = {
        valuesChange: 'values_change',
        errorsChange: 'errors_touched_change'
    };
    function withForm(configOrComponent) {
    ...
    const withFormExtended = (config) => (Component) => class LiteForm extends Component {
        constructor() {
            ...
            this.handleSubmit = (e) => {
                ...
                if (this.isValid) {
                    ...
                    this._touched = Object.keys(this._validationSchema).reduce((obj, fieldName) => {
        ...
        set _values(values) {
            this.values = values;
            this.dispatchEvent(new CustomEvent(EVENTS.valuesChange, {
        ...
        set _errors(errors) {
            this.errors = errors;
            this.dispatchEvent(new CustomEvent(EVENTS.errorsChange, {
    ...
    const getFormClass = (element) => {
        ...
        if (!host)
            throw new Error('Lite-form not found');
        return host[IS_LITE_FORM] ? host : getFormClass(host);
    };
    const withFormClass = (Component) => {
        return class extends Component {
            connectedCallback() {
                var _a;
                // Optimization - it don't need to looking for form if class already have it
                this._formClass = (_a = this._formClass) !== null && _a !== void 0 ? _a : getFormClass(this);
                super.connectedCallback && super.connectedCallback();
            }
        };
    };
    ...
    const withValue = (Component) => {
        return class extends withFormClass(Component) {
            ...
            connectedCallback() {
                ...
                this._formClass.addEventListener(EVENTS.valuesChange, this._onFormValuesChange.bind(this));
            }
            disconnectedCallback() {
                super.disconnectedCallback && super.disconnectedCallback();
                this._formClass.removeEventListener(EVENTS.valuesChange, this._onFormValuesChange);
            }
        };
    };

    function withField(configOrComponent) {
        if (typeof configOrComponent === 'object') {
    ...
    class ExwcInput extends LitElement {
    ...
    class ExwcInput$1 extends LitElement {
      createRenderRoot() {
        return this
      }
    ...
    customElements.define('error-message', withError(ExwcInput$1));
    ...
    class LiteForm$1 extends HTMLFormElement {
      connectedCallback() {
    ...
    LiteForm$1 = withForm(LiteForm$1);

    customElements.define('native-form', LiteForm$1, { extends: 'form' });
    ...
    customElements.define('wrapped-inputs-form', enhance$1(WrappedInputsForm));
